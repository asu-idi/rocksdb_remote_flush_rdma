...
// ConSharedArena is an implementation of Allocator class. For a request of
// small size, it allocates a block with pre-defined block size. For a request
// of big size, it uses malloc to directly get the requested size.
...
class ConSharedArena : public BasicArena {
 public:
  ...
  explicit ConSharedArena(size_t block_size = kMinBlockSize,
...
inline char* ConSharedArena::Allocate(size_t bytes) {
