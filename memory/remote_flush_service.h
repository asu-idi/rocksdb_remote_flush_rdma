...
private:
	std::vector<int> sock_connect(const char *servername, int port, size_t conn_cnt);
	int sock_sync_data(int sock, int xfer_size, const char *local_data, char *remote_data);
	int post_send(int idx, size_t msg_size, ibv_wr_opcode opcode, long long local_offset, long long remote_offset);
...
public:
	...
	int rdma_read(int idx, size_t msg_size, long long local_offset, long long remote_offset) {
		return post_send(idx, msg_size, IBV_WR_RDMA_READ, local_offset, remote_offset);
	}
	int rdma_write(int idx, size_t msg_size, long long local_offset, long long remote_offset) {
		return post_send(idx, msg_size, IBV_WR_RDMA_WRITE, local_offset, remote_offset);
	}
