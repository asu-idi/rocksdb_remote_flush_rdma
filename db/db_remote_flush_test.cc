...
class DBRemoteFlushTest : public DBTestBase {
 public:
  DBRemoteFlushTest()
...
class DBFlushDirectIOTest : public DBRemoteFlushTest,
 ...
 public:
  DBFlushDirectIOTest() : DBRemoteFlushTest() {}
};

class DBAtomicFlushTest : public DBRemoteFlushTest,
                          public ::testing::WithParamInterface<bool> {
 public:
  DBAtomicFlushTest() : DBRemoteFlushTest() {}
};

// We had issue when two background threads trying to flush at the same time,
// only one of them get committed. The test verifies the issue is fixed.
TEST_F(DBRemoteFlushTest, FlushWhileWritingManifest) {
  Options options;
  ...
  SyncPoint::GetInstance()->LoadDependency(
      {{"VersionSet::LogAndApply:WriteManifest",
        "DBRemoteFlushTest::FlushWhileWritingManifest:1"},
  ...
  TEST_SYNC_POINT("DBRemoteFlushTest::FlushWhileWritingManifest:1");
  ASSERT_OK(Put("bar", "v"));
  ASSERT_OK(dbfull()->Flush(no_wait));
...
// Github issue: #4151
TEST_F(DBRemoteFlushTest, SyncFail) {
  ...
  SyncPoint::GetInstance()->LoadDependency(
      {{"DBRemoteFlushTest::SyncFail:1", "DBImpl::SyncClosedLogs:Start"},
       {"DBImpl::SyncClosedLogs:Failed", "DBRemoteFlushTest::SyncFail:2"}});
  SyncPoint::GetInstance()->EnableProcessing();
  ...
  TEST_SYNC_POINT("DBRemoteFlushTest::SyncFail:1");
  TEST_SYNC_POINT("DBRemoteFlushTest::SyncFail:2");
...
TEST_F(DBRemoteFlushTest, SyncSkip) {
  Options options = CurrentRemoteOptions();
  SyncPoint::GetInstance()->LoadDependency(
      {{"DBRemoteFlushTest::SyncSkip:1", "DBImpl::SyncClosedLogs:Skip"},
       {"DBImpl::SyncClosedLogs:Skip", "DBRemoteFlushTest::SyncSkip:2"}});
  SyncPoint::GetInstance()->EnableProcessing();
  ...
  TEST_SYNC_POINT("DBRemoteFlushTest::SyncSkip:1");
  TEST_SYNC_POINT("DBRemoteFlushTest::SyncSkip:2");
...
TEST_F(DBRemoteFlushTest, FlushInLowPriThreadPool) {
...
TEST_F(DBRemoteFlushTest, CloseDBWhenFlushInLowPri) {
  Options options = CurrentRemoteOptions();
...
TEST_F(DBRemoteFlushTest, ManualFlushWithMinWriteBufferNumberToMerge) {
  Options options = CurrentRemoteOptions();
  ...
  SyncPoint::GetInstance()->LoadDependency(
      {{"DBImpl::BGWorkFlush",
        "DBRemoteFlushTest::ManualFlushWithMinWriteBufferNumberToMerge:1"},
       {"DBRemoteFlushTest::ManualFlushWithMinWriteBufferNumberToMerge:2",
...
TEST_F(DBRemoteFlushTest, ScheduleOnlyOneBgThread) {
  Options options = CurrentRemoteOptions();
  Reopen(options);
...
TEST_F(DBRemoteFlushTest, StatisticsGarbageBasic) {
  Options options = CurrentRemoteOptions();
...
TEST_F(DBRemoteFlushTest, StatisticsGarbageInsertAndDeletes) {
  Options options = CurrentRemoteOptions();
  options.statistics = CreateDBStatistics();
  options.statistics->set_stats_level(StatsLevel::kAll);
  options.create_if_missing = true;
...
TEST_F(DBRemoteFlushTest, StatisticsGarbageRangeDeletes) {
  Options options = CurrentRemoteOptions();
  options.statistics = CreateDBStatistics();
  options.statistics->set_stats_level(StatsLevel::kAll);
  options.create_if_missing = true;
...
class TestFlushListener : public EventListener {
 public:
  TestFlushListener(Env* env, DBRemoteFlushTest* test)
...
TEST_F(DBRemoteFlushTest, FixFlushReasonRaceFromConcurrentFlushes) {
  Options options = CurrentRemoteOptions();
  options.atomic_flush = true;
  options.disable_auto_compactions = true;
  CreateAndReopenWithCF({"cf1"}, options);
...
TEST_F(DBRemoteFlushTest, MemPurgeBasic) {
  Options options = CurrentRemoteOptions();
  ...
  ROCKSDB_NAMESPACE::SyncPoint::GetInstance()->SetCallBack(
      "DBImpl::RemoteFlushJob:MemPurgeSuccessful",
      [&](void* /*arg*/) { mempurge_count++; });
  ROCKSDB_NAMESPACE::SyncPoint::GetInstance()->SetCallBack(
      "DBImpl::RemoteFlushJob:SSTFileCreated",
      [&](void* /*arg*/) { sst_count++; });
  ROCKSDB_NAMESPACE::SyncPoint::GetInstance()->EnableProcessing();
...
// RocksDB lite does not support dynamic options
TEST_F(DBRemoteFlushTest, MemPurgeBasicToggle) {
  Options options = CurrentRemoteOptions();
...
// a regular Flush.
TEST_F(DBRemoteFlushTest, MemPurgeWithAtomicFlush) {
  Options options = CurrentRemoteOptions();
...
TEST_F(DBRemoteFlushTest, MemPurgeDeleteAndDeleteRange) {
  Options options = CurrentRemoteOptions();
...
TEST_F(DBRemoteFlushTest, MemPurgeAndCompactionFilter) {
  Options options = CurrentRemoteOptions();
...
TEST_F(DBRemoteFlushTest, DISABLED_MemPurgeWALSupport) {
  Options options = CurrentRemoteOptions();
...
TEST_F(DBRemoteFlushTest, MemPurgeCorrectLogNumberAndSSTFileCreation) {
  ...
  ROCKSDB_NAMESPACE::SyncPoint::GetInstance()->SetCallBack(
      "RemoteFlushJob::WriteLevel0Table:num_memtables", [&](void* arg) {
...
TEST_F(DBRemoteFlushTest, FlushError) {
  Options options;
  options.server_remote_flush = 1;
  options.server_use_remote_flush = true;
  std::unique_ptr<FaultInjectionTestEnv> fault_injection_env(
...
TEST_F(DBRemoteFlushTest, ManualFlushFailsInReadOnlyMode) {
...
TEST_F(DBRemoteFlushTest, CFDropRaceWithWaitForFlushMemTables) {
  Options options = CurrentRemoteOptions();
  options.create_if_missing = true;
  CreateAndReopenWithCF({"pikachu"}, options);
  SyncPoint::GetInstance()->DisableProcessing();
  SyncPoint::GetInstance()->LoadDependency(
      {{"DBImpl::FlushMemTable:AfterScheduleFlush",
        "DBRemoteFlushTest::CFDropRaceWithWaitForFlushMemTables:BeforeDrop"},
       {"DBRemoteFlushTest::CFDropRaceWithWaitForFlushMemTables:AfterFree",
...
TEST_F(DBRemoteFlushTest, FireOnFlushCompletedAfterCommittedResult) {
  ...
  SyncPoint::GetInstance()->LoadDependency(
      {{"DBImpl::FlushMemTableToOutputFile:AfterPickMemtables",
        "DBRemoteFlushTest::FireOnFlushCompletedAfterCommittedResult:"
        "WaitFirst"},
       {"DBImpl::FlushMemTableToOutputFile:Finish",
        "DBRemoteFlushTest::FireOnFlushCompletedAfterCommittedResult:"
        "WaitSecond"}});
  SyncPoint::GetInstance()->SetCallBack(
      "RemoteFlushJob::WriteLevel0Table", [&listener](void* arg) {
  ...
  TEST_SYNC_POINT(
      "DBRemoteFlushTest::FireOnFlushCompletedAfterCommittedResult:WaitFirst");
...
TEST_F(DBRemoteFlushTest, FlushWithChecksumHandoff1) {
  ...
  // unrecoverable error.
  SyncPoint::GetInstance()->SetCallBack("RemoteFlushJob::Start", [&](void*) {
    fault_fs->SetChecksumHandoffFuncType(ChecksumType::kxxHash);
  });
  ASSERT_OK(Put("key3", "value3"));
...
TEST_F(DBRemoteFlushTest, FlushWithChecksumHandoff2) {
  if (mem_env_ || encrypted_env_) {
    ROCKSDB_GTEST_SKIP("Test requires non-mem or non-encrypted environment");
    return;
  }
...
TEST_F(DBRemoteFlushTest, FlushWithChecksumHandoffManifest1) {
  if (mem_env_ || encrypted_env_) {
    ROCKSDB_GTEST_SKIP("Test requires non-mem or non-encrypted environment");
    return;
  }
...
TEST_F(DBRemoteFlushTest, FlushWithChecksumHandoffManifest2) {
  if (mem_env_ || encrypted_env_) {
    ROCKSDB_GTEST_SKIP("Test requires non-mem or non-encrypted environment");
    return;
  }
...
TEST_F(DBRemoteFlushTest, PickRightMemtables) {
  Options options = CurrentRemoteOptions();
  DestroyAndReopen(options);
  options.create_if_missing = true;
...
class DBRemoteFlushTestBlobError
    : public DBRemoteFlushTest,
      public testing::WithParamInterface<std::string> {
 public:
  DBRemoteFlushTestBlobError() : sync_point_(GetParam()) {}

  std::string sync_point_;
};

INSTANTIATE_TEST_CASE_P(DBRemoteFlushTestBlobError, DBRemoteFlushTestBlobError,
...
TEST_F(DBRemoteFlushTest, TombstoneVisibleInSnapshot) {
  class SimpleTestFlushListener : public EventListener {
   public:
    explicit SimpleTestFlushListener(DBRemoteFlushTest* _test) : test_(_test) {}
    ...
    DBRemoteFlushTest* test_ = nullptr;
...
TEST_P(DBAtomicFlushTest, BgThreadNoWaitAfterManifestError) {
  ...
  auto fault_injection_env = std::make_shared<FaultInjectionTestEnv>(env_);
  Options options = GetRemoteEnabledOptions();
  options.create_if_missing = true;
  options.atomic_flush = true;
  options.env = fault_injection_env.get();
